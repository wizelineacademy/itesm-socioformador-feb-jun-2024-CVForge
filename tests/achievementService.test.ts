    ...
    achievement: {
        findUnique: jest.fn().mockImplementation((query) => {
            if (query.where.achievement_id === '1') {
                return Promise.resolve({ achievement_id: '1', name: 'First Achievement', description: 'Achieved something important' });
            } else {
        ...
        findMany: jest.fn().mockImplementation((query = {}) => {
            ...
            if (where && where.professional_info_id === 'info1') {
                return Promise.resolve([
                    { achievement_id: '1', professional_info_id: 'info1', name: 'First Achievement', description: 'Achieved something important' },
                    { achievement_id: '3', professional_info_id: 'info1', name: 'Third Achievement', description: 'Achieved yet another thing' }
                ]);
            } else if (!where) {
                // Returns all achievements when there is no filtering condition
                return Promise.resolve([
                    { achievement_id: '1', name: 'First Achievement', description: 'Achieved something important' },
                    { achievement_id: '2', name: 'Second Achievement', description: 'Achieved another thing' }
                ]);
            } else {
                return Promise.resolve([]); // Returns an empty array for any other professional_info_id
            }
        }),
        create: jest.fn().mockImplementation((data) => Promise.resolve({ achievement_id: '1', ...data.data })),
    },
}));
...
describe('Achievement Services', () => {
    ...
    test('createAchievement should add a new achievement', async () => {
        ...
        const achievement = await achievementService.createAchievement(newAchievementData);
    ...
    test('findAchievementById should return an achievement', async () => {
        ...
        const achievement = await achievementService.findAchievementById(achievementId);
    ...
    test('findAchievementByProfessionalInfoId should return achievements for a given professional info ID', async () => {
        ...
        const expectedAchievements = [
            { achievement_id: '1', professional_info_id: professionalInfoId, name: 'First Achievement', description: 'Achieved something important' },
            { achievement_id: '3', professional_info_id: professionalInfoId, name: 'Third Achievement', description: 'Achieved yet another thing' }
        ];

        const achievements = await achievementService.findAchievementByProfessionalInfoId(professionalInfoId);
