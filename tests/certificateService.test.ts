  ...
  certificate: {
    create: jest
      ...
      .mockImplementation((data) =>
        Promise.resolve({ certificate_id: '1', ...data.data }),
      ),
    ...
    findMany: jest.fn().mockImplementation((options = {}) => {
      ...
      if (where && where.professional_info_id) {
        ...
        return Promise.resolve([
          {
            certificate_id: '2',
            professional_info_id: where.professional_info_id,
            name: 'Advanced Testing',
            issued_by: 'Advanced Body',
          },
        ])
...
describe('Certificate Services', () => {
  ...
  test('createCertificate should add a new certificate', async () => {
    ...
    const certificate =
      await certificateService.createCertificate(newCertificateData)
  ...
  test('findCertificateById should return a certificate', async () => {
    ...
    const certificate =
      await certificateService.findCertificateById(certificateId)
  ...
  test('findCertificateByProfessionalInfoId should return certificates for a given professional info ID', async () => {
    ...
    const certificates =
      await certificateService.findCertificateByProfessionalInfoId(
        professionalInfoId,
      )
